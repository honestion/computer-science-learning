...
``` 
so I can use .get() to count elements is a list dublicated elemets
```python
lst = [0, 1, .1, 2]
counts = {}
for elem in lst:
    counts[elem] = lst.get(elem, 0) + 1 # .get() takes index(start from zero) -> the 1 element was 1 time
```
## for loop can be forcrbly started with or '[None]'
